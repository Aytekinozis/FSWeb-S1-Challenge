...
function indekseGoreFenomen(dizi, indeks) {
  const metin = `${indeks}. indekste bulunan fenomen: ${dizi[indeks].profile}`;
  return metin
}
...
function fenomenEkle(dizi,sirano,profilismi,takipcisayisi,gonderisayisi,platformadi) {
  const yenidizi = [];
 for (let i = 0; i < dizi.length; i++) {
